        ...
        return new Promise(function (resolve, reject) {
            async.auto({
                webshotIa: function (next, results) {
                    generateWebShot(next, 'ia', flattenObj(data.iaObj));
                },

                webshotSFC: function (next, results) {
                    generateWebShot(next, 'sfc', flattenObj(data.sfcObj));
                },

                webshotIT: function (next, results) {
                    generateWebShot(next, 'it', flattenObj(data.itObj));
                },

                webshotPSF: function (next, results) {
                    generateWebShot(next, 'psf', flattenObj(data.psfObj));
                },

                webshotRA: function (next, results) {
                    generateWebShot(next, 'ra', flattenObj(data.raObj));
                },

                webshotSSO: function (next, results) {
                    generateWebShot(next, 'sso', flattenObj(data.ssoObj));
                },

                webshotTTR: function (next, results) {
                    generateWebShot(next, 'ttr', flattenObj(data.ttrObj));
                },

                webshotAsset: function (next, results) {
                    generateWebShot(next, 'aa', flattenObj(data.aaObj));
                },
